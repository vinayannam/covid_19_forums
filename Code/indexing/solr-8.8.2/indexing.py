...
def formatPosts(postsObj):
    ...
    for i in range(len(postsObj)):
        ...
        newMap['author'] = postsObj[i][1][3]
        newMap['symptoms'] = postsObj[i][1][4]
        newMap['treatments'] = postsObj[i][1][5]
        newMap['drugs'] = postsObj[i][1][6]
        newMap['bodyparts'] = postsObj[i][1][7]
        l1.append(newMap)
...
#            print(res.keys())
            ...
            reply_dict[post_link] = (replies_count, title, content, author, *post_keywords.values())
#            print(reply_dict)
